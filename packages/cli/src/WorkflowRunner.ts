...
export class WorkflowRunner {
	...
	async run(data: IWorkflowExecutionDataProcess, loadStaticData?: boolean, realtime?: boolean): Promise<string> {
		...
		console.log("------run  workFlow------"+JSON.stringify(data));

		console.log("------run  workFlow----executionsMode--"+executionsMode);

		console.log("------run  workFlow----executionsProcess--"+executionsProcess);
		...
		console.log("------run  workFlow----executionId--"+executionId);

		const externalHooks = ExternalHooks();
		if (externalHooks.exists('workflow.postExecute')) {

			console.log("------run  workFlow------workflow.postExecute---");

			this.activeExecutions.getPostExecutePromise(executionId)
				.then(async (executionData) => {
					console.log("------run  workFlow------executionData---"+JSON.stringify(executionData));
